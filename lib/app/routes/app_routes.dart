part of 'app_pages.dart';
// DO NOT EDIT. This is code generated via package:get_cli/get_cli.dart

abstract class Routes {
  Routes._();

  static const FIRST_PAGE = _Paths.FIRST_PAGE;
  static const SECOND = _Paths.SECOND;
  static const SECOND_PAGE = _Paths.SECOND_PAGE;
  static const WELCOME_PAGE = _Paths.WELCOME_PAGE;
  static const LOGIN_PAGE = _Paths.LOGIN_PAGE;
  static const REGISTER_PAGE = _Paths.REGISTER_PAGE;
  static const HOME_PAGE = _Paths.HOME_PAGE;
  static const VERIFICATION_PAGE = _Paths.VERIFICATION_PAGE;
  static const FORGET_PASSWORD_PAGE = _Paths.FORGET_PASSWORD_PAGE;
  static const VERIFICATION_PAGE_2 = _Paths.VERIFICATION_PAGE_2;
  static const CREATE_NEW_PASSWORD_PAGE = _Paths.CREATE_NEW_PASSWORD_PAGE;
  static const ALL_TEMPLATES_PAGE = _Paths.ALL_TEMPLATES_PAGE;
  static const MY_LIBRARY_PAGE = _Paths.MY_LIBRARY_PAGE;
  static const TEMPLATES_GALLERY_PAGE = _Paths.TEMPLATES_GALLERY_PAGE;
  static const GRATIS_PAGE = _Paths.GRATIS_PAGE;
  static const PROFILE_PAGE = _Paths.PROFILE_PAGE;
  static const MY_PURCHASES_PAGE = _Paths.MY_PURCHASES_PAGE;
  static const MANAGE_PACKAGE_PAGE = _Paths.MANAGE_PACKAGE_PAGE;
  static const SPLASH = _Paths.SPLASH;
  static const EDIT_PROFILE = _Paths.EDIT_PROFILE;
  static const GRATIVID_CHOICE = _Paths.GRATIVID_CHOICE;
  static const BIRTHDAY = _Paths.BIRTHDAY;
  static const NOTIFICATION = _Paths.NOTIFICATION;
  static const ANNIVERSARY = _Paths.ANNIVERSARY;
  static const THANKYOU = _Paths.THANKYOU;
  static const CONGRATULATION = _Paths.CONGRATULATION;
  static const WEDDING = _Paths.WEDDING;
  static const FRIENDSHIP = _Paths.FRIENDSHIP;
  static const LOVE = _Paths.LOVE;
  static const NEW_BABY = _Paths.NEW_BABY;
  static const RETIREMENT = _Paths.RETIREMENT;
  static const GRADUATION = _Paths.GRADUATION;
  static const SENT_ITEMS = _Paths.SENT_ITEMS;
  static const SCHEDULE_ITEMS = _Paths.SCHEDULE_ITEMS;
  static const DRAFT_ITEMS = _Paths.DRAFT_ITEMS;
}

abstract class _Paths {
  _Paths._();

  static const FIRST_PAGE = '/first-page';
  static const SECOND = '/second';
  static const SECOND_PAGE = '/second-page';
  static const WELCOME_PAGE = '/welcome-page';
  static const LOGIN_PAGE = '/login-page';
  static const REGISTER_PAGE = '/register-page';
  static const HOME_PAGE = '/home-page';
  static const VERIFICATION_PAGE = '/verification-page';
  static const FORGET_PASSWORD_PAGE = '/forget-password-page';
  static const VERIFICATION_PAGE_2 = '/verification-page-2';
  static const CREATE_NEW_PASSWORD_PAGE = '/create-new-password-page';
  static const ALL_TEMPLATES_PAGE = '/all-templates-page';
  static const MY_LIBRARY_PAGE = '/my-library-page';
  static const TEMPLATES_GALLERY_PAGE = '/templates-gallery-page';
  static const GRATIS_PAGE = '/gratis-page';
  static const PROFILE_PAGE = '/profile-page';
  static const MY_PURCHASES_PAGE = '/my-purchases-page';
  static const MANAGE_PACKAGE_PAGE = '/manage-package-page';
  static const SPLASH = '/splash';
  static const EDIT_PROFILE = '/edit-profile';
  static const GRATIVID_CHOICE = '/grativid-choice';
  static const BIRTHDAY = '/birthday';
  static const NOTIFICATION = '/notification';
  static const ANNIVERSARY = '/anniversary';
  static const THANKYOU = '/thankyou';
  static const CONGRATULATION = '/congratulation';
  static const WEDDING = '/wedding';
  static const FRIENDSHIP = '/friendship';
  static const LOVE = '/love';
  static const NEW_BABY = '/new-baby';
  static const RETIREMENT = '/retirement';
  static const GRADUATION = '/graduation';
  static const SENT_ITEMS = '/sent-items';
  static const SCHEDULE_ITEMS = '/schedule-items';
  static const DRAFT_ITEMS = '/draft-items';
}
